...
for ft_type_num = 1:len(ft_types)
    ...
    for seednum = 1:len(seeds)
        ...
        if strcmp(ft_type,'no')
            ...
            PLCC_no_ft(seednum) = PLCC.max;
            SROCC_no_ft(seednum) = SROCC.max;
        elseif strcmp(ft_type,'correct')
            for testnum = 1:len(test_types)
                ...
                if strcmp(test_type,'independent')
                    PLCC_correct_ft_independent_test(seednum) = PLCC.avg;
                    SROCC_correct_ft_independent_test(seednum) = SROCC.avg;
                else
                    tmp_PLCC = zeros(5,1);
                    tmp_SROCC = zeros(5,1);
                    for realization = 1:5
                        tmp_PLCC(realization) = PLCC{realization}.avg;
                        tmp_SROCC(realization) = SROCC{realization}.avg;
                    end
            ...
            for testnum = 1:len(test_types)
                ...
                load([results_path 'konvid1k_iv3_results_' ft_type '_ft_' test_type '_tests_' num2str(baseseed) '.mat'],'PLCC','SROCC')
                if strcmp(test_type,'independent')
                    PLCC_faulty_ft_independent_test(seednum) = PLCC.avg;
                    SROCC_faulty_ft_independent_test(seednum) = SROCC.avg;
                else
                    tmp_PLCC = zeros(5,1);
                    tmp_SROCC = zeros(5,1);
                    for realization = 1:5
